...
class PostTest extends TestCase
{
  ...
  public function testCreatingAPostWithNullAsQuantity()
  {
    ...
    // Create the post!
    $response = $this->withAccessToken($signup->northstar_id, 'user')->postJson(
      'api/v3/posts',
      [
        'northstar_id' => $signup->northstar_id,
        'campaign_id' => $signup->campaign_id,
  ...
  public function testPostsIndexWithAnonymousPosts()
  {
    ...
    $response = $this->withAccessToken($anonymousPost->northstar_id)->getJson(
      'api/v3/posts'
    );
    $response->assertStatus(200);
    $this->assertEquals(
  ...
  public function testPostsIndexAsVoterRegistrationReferrer()
  {
    ...
    factory(Post::class)
      ->states('voter-reg', 'step-1')
      ->create(['referrer_user_id' => $this->faker->unique()->northstar_id]);
