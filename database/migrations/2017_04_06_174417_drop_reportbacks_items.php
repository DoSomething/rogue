  ...
  public function down()
  {
    Schema::create('reportbacks', function (Blueprint $table) {
      ...
      $table
        ->integer('campaign_id')
        ->index()
        ->comment('The campaign node id that the user has reported back for.');
      $table
        ->integer('campaign_run_id')
        ->index()
        ->nullable()
        ->comment('The campaign run id that the user reported back for.');
      $table
        ->integer('quantity')
        ->comment('The quantity of reportback_nouns reportback_verbed.');
      $table
    ...
    Schema::create('reportback_logs', function (Blueprint $table) {
      ...
      $table
        ->string('files')
        ->nullable()
        ->comment('Comma separated list of file fids attached to reportback.');
      $table
        ->integer('num_files')
      ...
      $table
        ->text('reason')
        ->nullable()
        ->comment('The reason the reoportback item was flagged/promoted');
      $table->timestamps();
